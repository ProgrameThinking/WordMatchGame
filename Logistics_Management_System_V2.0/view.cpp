 ...
 * @LastEditTime: 2023-01-06 18:04:28
...
void view:: postmanlogin()
{
    ...
    string sql="select * from postman where username='"+username+"' and password='"+password+"'";
    MYSQL_RES* res=mysql.QueryData(sql.c_str());
    if(0==res->row_count)
    {
        cout<<"用户名或者密码错误！"<<endl;
...
void view:: postmanRegister()
{
    ...
    string sql="insert into postman values('"+p.getUsername()+"','"+p.getPassword()+"','"+p.getRealName()+"','"+p.getphoneNumber()+"',"+to_string(p.getAccountBalance())+","+to_string(p.getState())+")";
    mysql.ExecuteSql(sql.c_str());        /*执行sql语句*/
    mysql.FreeConnect();                  /*关闭数据库链接*/
    cout<<"注册成功！"<<endl;
}
